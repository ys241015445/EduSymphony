...
export default function TeachingFeedback({ lessonId, lessonTitle }: TeachingFeedbackProps) {
  const t = useT()
  ...
  const handleSaveFeedback = async () => {
    ...
    try {
      ...
      await api.post(`/api/v1/lessons/${lessonId}/feedback`, form)
  ...
  const handleGenerateNext = async () => {
    ...
    try {
      ...
      const res = await api.post(`/api/v1/lessons/${lessonId}/next-lesson`, form)
      navigate(`/lesson/${res.data.id}/process`)
    } catch (e) {
