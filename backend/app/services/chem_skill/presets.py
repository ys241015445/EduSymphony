# -*- coding: utf-8 -*-
"""
presets.py — 反应预设注册表（改造自 edulab edu-chem-reaction 的 scripts/generate.py）。

每个 build_* 返回一个"作者 spec"，reaction_kernel.assemble_data 负责校验（配平/原子守恒/
映射双射/键差）与装配。本文件去掉了原版的文件写出与 CLI，改为对外提供 generate_reaction_html。
"""

from . import reaction_kernel as K
...
# 每个预设的中文名 + 触发关键词（供检测/分类；关键词命中即可直接选中，无需 AI）。
REACTION_META = {
    "combustion_ch4": {"name": "甲烷的燃烧", "keywords": ["甲烷", "ch4", "甲烷燃烧", "天然气燃烧"]},
    "combustion_h2": {"name": "氢气的燃烧", "keywords": ["氢气燃烧", "氢气的燃烧", "h2燃烧", "氢燃烧"]},
    "electrolysis_water": {"name": "电解水", "keywords": ["电解水", "水的电解", "electrolysis"]},
    "redox_na_cl2": {"name": "钠与氯气（氧化还原）", "keywords": ["钠", "氯气", "钠和氯", "nacl 生成", "氧化还原 钠"]},
    "esterification": {"name": "酯化反应", "keywords": ["酯化", "乙酸乙酯", "酯化反应", "esterification"]},
    "glucose_combustion": {"name": "葡萄糖的燃烧/氧化", "keywords": ["葡萄糖", "呼吸作用", "glucose", "c6h12o6"]},
}


def list_presets() -> list[dict]:
    """返回 [{key, name, keywords}] 供前端/分类使用。"""
    return [{"key": k, **REACTION_META.get(k, {"name": k, "keywords": []})} for k in REGISTRY]


def generate_reaction_html(key: str) -> str:
    """预设 key → 完整可交互 HTML（确定性：sympy 配平 + 内核校验 + 模板渲染）。"""
    if key not in REGISTRY:
        raise KeyError(f"unknown reaction preset: {key}")
