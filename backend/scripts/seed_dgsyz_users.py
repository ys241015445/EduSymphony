"""
一次性种子脚本：在 public.users 中创建/更新 dgsyz01–dgsyz15 普通账号。

权限：新建教案全流程 + 下载（can_export、can_next_lesson=true）；
课程工具/模板/大学/系列/珠科等模块保持关闭。
密码与用户名相同，使用 get_password_hash 写入 password_hash。

用法（在 backend 目录）：
    .\\venv\\Scripts\\python.exe scripts\\seed_dgsyz_users.py
"""
